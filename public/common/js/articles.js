...
$(document).ready(function(){
    ...
    if($(".shezhitoutiao").length > 0){
        $(".chang").hover(function(){
            if($(this).find(".quxiaotoutiao").closest("div").hasClass("hidden")){
                $(this).find(".shezhitoutiao").closest("div").removeClass("hidden");
            }
            else{
                $(this).find(".quxiaotoutiao").removeClass("hidden");
            }
        },function(){
            $(this).find(".shezhitoutiao").closest("div").addClass("hidden");
            $(this).find(".quxiaotoutiao").addClass("hidden");
        });
        $(".shezhitoutiao").on("click", function(){
            var tobj = $(this);
            var tdiv = $(this).closest("div");
            tobj.next().removeClass("hidden");
            $.post("toutiao", { id: tobj.data("id")},
                function(data){
                    tobj.next().addClass("hidden");
                    $(".quxiaotoutiao").closest("div").addClass("hidden");
                    tdiv.addClass("hidden").prev().removeClass("hidden");
                });
        });
        $(".quxiaotoutiao").on("click", function(){
            var tobj = $(this);
            var tdiv = $(this).closest("div");
            tobj.next().removeClass("hidden");
            $.post("quxiaotoutiao", { id: tobj.data("id")},
                function(data){
                    tobj.next().addClass("hidden");
