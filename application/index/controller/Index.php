...
class Index extends Common
{
    ...
    {
        $perPageShow = 10;
        if(isset($this->options_spare['homeShows']))
        {
            $perPageShow = $this->options_spare['homeShows'];
        }
        if(Request::instance()->has('page','post'))
        {
            $order = 'istop desc,post_modified desc';
            Hook::add('change_blog_order',$this->plugins);
            Hook::listen('change_blog_order',$order,$this->ccc);
            $page = Request::instance()->post('page');
            $getpage = Cache::get('index_page'.$page.'_'.$this->lang);
            if($getpage == false)
            {
                $data = Db::view('posts','id,post_keywords as guanjianzi,post_content as zhengwen,post_title as biaoti,post_excerpt as zhaiyao,post_modified as fabushijian,comment_count as pinglunshu,thumbnail as suolvetu,post_hits as yuedu,post_like as zan,istop as zhiding')
        ...
        $catfishBlog = Cache::get('catfishBlog_'.$page.'_'.$this->lang);
        if($catfishBlog == false)
        {
            ...
            Cache::tag('catfishBlog_'.$page)->set('catfishBlog_'.$page.'_'.$this->lang,$catfishBlog,3600);
        }
    ...
    public function article($id = 0)
    {
        if(empty($id) || $id == 'all')
        {
            if(Request::instance()->has('page','post'))
            {
                ...
                if($getpage == false)
                {
                    ...
                    $data = Db::view('posts','id,post_keywords as guanjianzi,post_content as zhengwenfz,post_title as biaoti,post_excerpt as zhaiyao,post_modified as fabushijian,comment_count as pinglunshu,thumbnail as suolvetu,post_hits as yuedu,post_like as zan,istop as zhiding')
                        ->view('users','user_nicename as nicheng,avatar as touxiang,sex as xingbie','users.id=posts.post_author')
                        ->where('post_status','=',1)
        ...
        else
        {
            ...
            $noArticle = false;
            $data = Db::view('posts','id,post_author,post_keywords as guanjianzi,post_source as laiyuan,post_content as zhengwen,post_title as biaoti,post_excerpt as zhaiyao,post_status,comment_status,post_modified as fabushijian,post_type,comment_count as pinglunshu,thumbnail as suolvetu,post_hits as yuedu,post_like as zan,istop as zhiding')
                ->view('users','user_login as yonghu,user_nicename as nicheng,avatar as touxiang,sex as xingbie','users.id=posts.post_author')
                ->where('posts.id',$id)
                ->where('status','=',1)
                ->find();
    ...
    public function category($id)
    {
        if(Request::instance()->has('page','post'))
        {
            ...
            if($getpage == false)
            {
                $data = Db::view('term_relationships','term_id')
                    ->view('posts','id,post_keywords as guanjianzi,post_content as zhengwenfz,post_title as biaoti,post_excerpt as zhaiyao,post_modified as fabushijian,post_type as type,comment_count as pinglunshu,thumbnail as suolvetu,post_hits as yuedu,post_like as zan,istop as zhiding','posts.id=term_relationships.object_id')
                    ->view('users','user_nicename as nicheng,avatar as touxiang,sex as xingbie','users.id=posts.post_author')
        ...
        Hook::add('order_category',$this->plugins);
        Hook::listen('order_category',$order,$this->ccc);
        $menuPath = $this->menuPath($id,'category');
        $termid = $id;
        if(isset($this->options_spare['includeSubcategories']) && $this->options_spare['includeSubcategories'] == 1)
        {
            $subcategory = Cache::get('allsubcategories'.$id);
    ...
    public function page($id)
    {
        ...
        if(!empty($catid)){
            ...
            $cdata = Cache::get('category'.$termid.'@'.$page.'_'.$this->lang);
            if($cdata == false)
            {
                $cdata = Db::view('term_relationships','term_id')
                    ...
                    ->order('istop desc,'.$order['name'].' '.$order['way'])
                    ->paginate($myxs);
                Cache::set('category'.$termid.'@'.$page.'_'.$this->lang,$cdata,3600);
            }
    ...
    public function rili()
    {
        $nian = date("Y");
        $yue = date("m");
        if(Request::instance()->has('nianyue','post')){
            $nianyue = Request::instance()->post('nianyue');
            $tmparr = explode('-', $nianyue);
            $nian = $tmparr[0];
            $yue = $tmparr[1];
        }
        if(Request::instance()->has('nian','post')){
            $nian = Request::instance()->post('nian');
        }
        if(Request::instance()->has('yue','post')){
            $yue = Request::instance()->post('yue');
        }
        $riguidang = $this->getrili($yue, $nian);
        $this->assign('riguidang', $riguidang);
        $data_options = Cache::get('options');
        if($data_options == false)
        {
            $data_options = Db::name('options')->where('autoload',1)->field('option_name,option_value')->select();
        ...
        if(Request::instance()->isMobile() && is_file(APP_PATH.'../public/'.$template.'/part/mobile/rili.html'))
        {
            $htmls = $this->fetch(APP_PATH.'../public/'.$template.'/part/mobile/rili.html');
        }
        elseif(is_file(APP_PATH.'../public/'.$template.'/part/rili.html'))
        {
            $htmls = $this->fetch(APP_PATH.'../public/'.$template.'/part/rili.html');
        }
        else{
            header('Content-Type: application/json');
            echo json_encode($riguidang);
            exit();
        }
        return $htmls;
    }
